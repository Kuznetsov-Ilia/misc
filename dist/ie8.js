...
(function (W, D) {
  ...
  var
    ...
    ElementPrototype = W.Element.prototype,
    TextPrototype = W.Text.prototype,
  ...
  commonTextContent(
    W.HTMLCommentElement.prototype,
    ElementPrototype,
    'nodeValue'
  );

  commonTextContent(
    W.HTMLScriptElement.prototype,
    null,
    'text'
  );
  ...
  commonTextContent(
    W.HTMLTitleElement.prototype,
    null,
    'text'
  );

  defineProperty(
    W.HTMLStyleElement.prototype,
    'textContent', (function (descriptor) {
    ...
    }(getOwnPropertyDescriptor(W.CSSStyleSheet.prototype, 'cssText')))
  );
...
*/
  ...
  defineProperties(TextPrototype, {
    addEventListener: {
      value: ElementPrototype.addEventListener
    },
    dispatchEvent: {
      value: ElementPrototype.dispatchEvent
    },
    removeEventListener: {
      value: ElementPrototype.removeEventListener
    }
  });
  ...
  defineProperties(
    W.Window.prototype, {
      ...
      dispatchEvent: {
        value: function (e) {
          var method = W['on' + e.type];
          return method ? method.call(W, e) !== false && !e.defaultPrevented : true;
        }
      },
      removeEventListener: {
        value: function (type, handler, capture) {
          var
            ontype = 'on' + type,
            handlers = (W[ontype] || Object)[SECRET],
            i = handlers ? find(handlers, handler) : -1;
          if (-1 < i) handlers.splice(i, 1);
        }
      }
  ...
  (function (styleSheets, HTML5Element, i) {
    for (i = 0; i < HTML5Element.length; i++) D.createElement(HTML5Element[i]);
    if (!styleSheets.length) D.createStyleSheet('');
    styleSheets[0].addRule(HTML5Element.join(','), 'display:block;');
  }(D.styleSheets, ['header', 'nav', 'section', 'article', 'aside', 'footer']));
  ...
  function onkeyup(e) {
    ...
    (e.srcElement || e.fromElement || D).dispatchEvent(evt);
  }
  ...
  function onReadyState(e) {
    if (!READYEVENTDISPATCHED && readyStateOK.test(
      ...
      D.detachEvent(ONREADYSTATECHANGE, onReadyState);
  ...
  function setTextContent(textContent) {
    ...
    if (textContent != null) {
      this.appendChild(D.createTextNode(textContent));
    }
  }
  ...
  function verify(self, e) {
    ...
    if (!e.target) {
      e.target = e.srcElement || e.fromElement || D;
    }
    if (!e.timeStamp) {
      e.timeStamp = (new Date).getTime();
    }
